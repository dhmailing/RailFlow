...
- **외부 웹 계정삭제 페이지**: 네이티브 앱이 아직 없으므로, 이 웹/PWA 앱 자체가 그 "외부 웹" 경로를 겸한다 — `/privacy` 페이지가 삭제 방법을 안내하고, 로그인 후 같은 도메인에서 즉시 삭제할 수 있다. **다만 이 요구사항을 실제로 충족했다고 단정하지 않는다** — Play Console 심사를 받은 적이 없고, 앱이 아직 배포되지 않아 심사 대상 URL로 등록되지도 않았다. 네이티브 앱을 실제로 제출하는 시점에 이 경로가 Google Play 정책의 최신 요건(문서화된 URL 안정성, 언어, 접근성 등)을 충족하는지 다시 확인·검증해야 한다.
