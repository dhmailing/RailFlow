...
| --- | --- | --- | --- | --- |
| **불투명 서버 세션(채택)** | `crypto.randomBytes` 토큰 + 서버측 세션 레코드 | 즉시 revocation(행 삭제), 서명 키 관리 불필요, 계정 삭제/kill switch가 단순 delete, Android Bearer 토큰으로도 바로 재사용 가능 | 매 요청마다 저장소 조회 필요(세션 검증에 DB/캐시 hit 1회) | 없음 |
...
**결론**: 이번 단계 제약(외부 프로젝트·유료 리소스 생성 금지) 아래에서는 불투명 서버 세션이 유일하게 지금 바로 구현 가능하면서 Android 확장성(Bearer 토큰)도 만족하는 선택지다. OAuth/외부 인증 서비스는 그 제약이 풀리면 재검토 대상이다.
