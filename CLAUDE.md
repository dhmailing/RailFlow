...
사용자는 iPad와 Windows PC를 함께 사용한다. 다만 **PC에 프로그램 설치나 터미널 실행이 가능한지는 아직 확정되지 않았으므로, 설치 가능한 개발 환경으로 가정하지 않는다.** 기본값은 설치·터미널 없이 진행하는 것이며, 터미널 명령 실행·로컬 서버 구동·Android Studio/Node 설치·파일의 수동 일괄 수정이 꼭 필요하면 요구하기 전에 가능한지 먼저 확인한다. 일반 브라우저로 할 수 있는 일(웹 화면 확인, 파일 내려받기 등)은 iPad·PC 어느 쪽이든 요청해도 된다. 결과는 완전한 파일 또는 내려받을 수 있는 전체 프로젝트 ZIP으로 제공한다.
